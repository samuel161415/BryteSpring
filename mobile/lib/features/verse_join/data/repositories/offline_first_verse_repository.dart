...
class OfflineFirstVerseRepository implements VerseJoinRepository {
  ...
  Future<Either<Failure, VerseJoinEntity>> joinVerse(String verseId) async {
    try {
      ...
      return Left(NetworkFailure('No internet connection and no cached verse data available'));
      ...
      if (isOnline) {
        try {
          ...
          if (response.statusCode == 200) {
            ...
            final verses = versesJson.map((json) => VerseJoinEntity.fromJson(json)).toList();
  ...
  Future<void> _removeFromJoinedVersesCache(String verseId) async {
    try {
      final cachedData = await localStorage.getCachedData('joined_verses');
      if (cachedData != null) {
        final List<dynamic> versesJson = List<dynamic>.from(cachedData['verses']);
